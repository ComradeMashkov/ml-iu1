...
def test_units_are_canonical():
    out = canonicalize({"gyro_dps": np.array([180.0]), ...})
    assert out["gyro_rad_s"][0] == pytest.approx(np.pi)
```
...
# Для равномерного downsampling сначала применяем anti-alias FIR:
...
def test_positive_lag_means_second_signal_is_delayed():
    pulse = np.r_[np.zeros(20), 1.0, np.zeros(40)]
    delayed = np.r_[np.zeros(3), pulse[:-3]]
    assert estimate_offset(pulse, delayed, 0.01, 0.1) == 0.03
```
