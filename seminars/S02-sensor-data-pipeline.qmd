...
## Измените окно — и увидьте повтор данных {.mission-lab-slide}

<iframe
  class="mission-frame"
  src="../interactive/flight-mission.html?view=window&embed=1"
  title="Интерактивная геометрия окна и перекрытия на телеметрии полёта"
  loading="lazy"
  allow="fullscreen"
></iframe>

::: {.notes}
**Тайминг:** 1:14–1:18. График уже приближен к манёвру на 03:00. Сначала показать исходные
четыре рамки при `length=2 с`, `stride=0.5 с`: окна почти лежат друг на друге, получаем
597 строк против 212 условно независимых фрагментов. Затем поставить `length=8 с`,
`stride=4 с`: рамки расширятся и разойдутся, счётчики изменятся на 74 и 42.

После этого оставить `length=8 с` и вернуть `stride=0.5 с`: рамки снова почти совпадут,
overlap станет около 94%, строк будет 585, но условно независимых — только 113. Видимый
эффект — меняются ширина и взаимный сдвиг рамок W−3…W₀, а не форма исходного сигнала.

Число «условно независимых» — учебная эвристика для визуального сравнения, не статистическая
оценка effective sample size.
:::
...
## Смените вопрос — и «качество» перестроится {.mission-lab-slide}

<iframe
  class="mission-frame"
  src="../interactive/flight-mission.html?view=split&embed=1"
  title="Интерактивное сравнение случайного, группового и временного split"
  loading="lazy"
  allow="fullscreen"
></iframe>

::: {.notes}
**Тайминг:** 1:19–1:23. Последовательно выбрать «окно», «полёт», «будущее». В первом режиме
красные и оранжевые клетки рассыпаны внутри каждой строки flight_id — соседние окна одного
полёта попали по разные стороны оценки. Во втором режиме F05 целиком становится validation,
F06 — test. В третьем цвета образуют вертикальные временные блоки, разделённые серым gap.

Значения F1 синтетические и показывают направление эффекта; обсуждать нужно изменение
эксплуатационного вопроса, а не конкретные сотые.
:::
