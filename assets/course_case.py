...
* a rare binary target: degradation within the next 30 minutes.
"""
...
def generate_course_case(
    *,
    ...
    The target is a simulated maintenance-event flag within a 30-minute
    horizon. Rows represent overlapping 20-second windows sampled every
    10 seconds. Labels and nuisance offsets are correlated inside a flight,
    which makes a random row split over-optimistic for flexible models.
    """
    ...
    for actuator_idx in range(n_actuators):
        ...
        for flight_idx in range(flights_per_actuator):
            ...
            regime = str(
                regime_names[(actuator_idx + 2 * flight_idx) % len(regime_names)]
            )
...
def course_holdout_masks(case: ActuatorCase) -> tuple[
    NDArray[np.bool_],
    NDArray[np.bool_],
    NDArray[np.bool_],
]:
